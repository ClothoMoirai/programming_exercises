...
def get_file_stats(filename: str) -> dict:
    # Return a dict with the size and, if there are multiple links, the hardlinks
    stat_dict = {}
...
def scan_directory(path: str) -> dict:
    ...
    files_dict = {'size': defaultdict(list), 'hardlinks': defaultdict(list)}
        ...
        for this_file in these_files:
            ...
            if isfile(this_file_abs_name):
                ...
                if 'hardlinks' in stats_dict:
                    files_dict['hardlinks'][stats_dict['hardlinks']].append(this_file_abs_name)
    return files_dict
...
def get_file_hashes(file_size_dict: dict) -> dict:
    ...
    file_hashes_dict = clear_single_entries(file_hashes_dict)
...
def main(my_dir: str) -> dict:
    ...
    displayprint(possible_dup_dict)
